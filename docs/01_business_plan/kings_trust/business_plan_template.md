# King’s Trust Business Plan Template

# Section One: Executive Summary

## Year 1 Trading Period

**This business plan focuses on Year 1 from:** `[Month Year]` to `[Month Year]`

---

## Business Name Checks

Before finalising your business name, research it on:

- Companies House
- UK trademark search
- Social media platforms
- Google search
- Domain name availability

Useful links / checks:

- Search for a trade mark - GOV.UK
- Get information about a company - GOV.UK
- Annex A: Sensitive words and expressions that require prior approval to use in a company or business name - GOV.UK
- Set up a business - GOV.UK
- Working for yourself - GOV.UK

---

## 1.1 Business Summary

### Explain your business idea

**What do you do, what are you going to sell, where, and to whom?**  

**zer0Gravity** is a drone technology business specialising in:
- aerial cinematography, 
- data capture
- advanced drone development 

providing high-quality drone footage for:
- real estate 
- events
- commercial clients

before expanding into drone-based mapping, surveying, and product development.


---

### Explain your business name

**Why did you choose this name?**  

- **zer0Gravity**

---

### Business structure

**What type of business have you chosen?**  
*Examples: sole trader, limited company, partnership.*  

The business will initially operate as a **sole trader**

---

### Reason for choosing this structure

This structure has been chosen due to its **simplicity**, **low startup costs**, and **flexibility** while establishing the business in its early stages

---

## 1.2 Business Aims

Use SMART goals for each aim.

| Timeframe | Business Aim | SMART Goal |
|---|---|---|
| **Short term: Year 1** | **1.** Build client base <br> **2.** Generate income <br> **3.** Develop portfolio <br> **4.** Begin R&D | **1.** Secure 5–10 paying clients <br> **2.** Earn **£1000** per month consistently <br> **3.** Build portfolio with 10 completed projects <br> **4.** Complete initial drone R&D prototype 
| **Mid-term: 3 years** |**1.** Expand services <br> **2.** Introduce data solutions <br>**3.** Develop products | **1.** Launch mapping services <br> **2.** Secure 3+ commercial clients <br> **3,** Develop and test 2 drone prototypes 
| **Long term: 5+ years** | **1.** AI integration <br> **2.** Manufacturing <br> **3.** Brand growth | **1.** Develop AI-assisted drone system <br> **2.** Launch drone product line <br> **3.** Establish recognised brand in UK 


---

## 1.3 Financial Summary

| Item | Amount / Answer |
|---|---|
| Turnover in the next 12 months of trading | **£ [12,000]** |
| Money left in the business at the end of this period | **£ [3,500]**|
| Total funding required to start the business |**£ [3,000]** |
| Personal contribution / test trading |**£ [200 + existing equipment]** |
| Grant funding required from King’s Trust | **£ [2,800]** |
| Loan funding required | **£ [0]**|
| Business mentoring support |** [Yes]** |

---

### Cost Plus Pricing

Example:  
I have decided not to include my labour costs in my cost plus pricing because the profit margin is high enough that I do not require this at this stage.

- My pricing takes into account operational costs such as travel, equipment use, batteries, and outsourced editing where required. At this stage, I am focused on building my portfolio and client base, so I am not fully factoring in my labour costs. However, all projects are priced to remain profitable while still being competitive within the market.

---

### Sales and Costs Forecast

Example:  
When I hire the media company in March 2022, as shown in my cashflow, I am confident we will be able to dramatically increase our sales in Year 2.

- In the first year, I expect gradual growth as I build my portfolio, online presence, and relationships with estate agents and local businesses. Drone cinematography will be the main source of income while I continue improving my technical capabilities and developing advanced drone systems for future expansion into more specialised services.

---

### Personal Survival Budget

Example:  
I have no rent or rates because I am still living at home with my parents rent free.

- I currently have low living costs, which allows me to focus on growing the business without immediate financial pressure. This gives me the flexibility to reinvest profits into equipment, learning, and business development during the first year.

---

### Cashflow

Example:  
I have no sales during July and August because schools are closed for summer holidays and this is when I am taking my holidays.

- My cashflow reflects gradual growth over time rather than immediate high income. Early months may have lower income while I focus on networking, portfolio building, and developing my services. Costs include travel, equipment maintenance, batteries, outsourced editing, and ongoing development tools required to improve service quality and reliability.

---

## 1.4 Your Business Name

**zer0Gravity**
---

## 1.5 Strapline

*Elevate Your Vision*

---

## 1.6 Elevator Pitch

- Zer0Gravity is a drone cinematography business delivering high-quality aerial photography and videography for real estate, events, and businesses. By combining creative storytelling with advanced drone technology, the business provides unique elevated perspectives while building toward future innovation in drone systems and design.
---

# Section Two: Owner’s Background

## 2.1 Why Do You Want to Run Your Own Business?

### Why have you decided to start your own business?

Example:  
I have always been interested in technology, creativity, and building things, and starting my own business allows me to combine these passions into something meaningful while creating my own opportunities.

- I have always been interested in technology, creativity, and building things, and starting my own business allows me to combine these passions into something meaningful while creating my own opportunities.
---

### Why is it the right choice for you?

Example:  
It allows me to take control of my career, apply my engineering background in a practical way, and grow at my own pace while developing both technical and creative skills.

- It allows me to apply my engineering background in a practical and creative way while continuously developing my technical skills and growing at my own pace.
---

### What drives you to make it a success?

- I am driven by the desire to build something from the ground up and turn my ideas into a real business that can grow into an innovative technology brand in the future.


---

### Skills, knowledge, experience, and transferable skills

- I have a degree in Electrical and Electronic Engineering, giving me a strong understanding of systems, problem-solving, and technology. I also have hands-on experience building and working with drones, including custom FPV systems and autonomous drone development using LiDAR technology.

- In addition, I have practical experience working as a technician in a laser and special effects company, where I developed teamwork, troubleshooting, and technical setup skills in fast-paced environments. I also have transferable skills in communication, organisation, adaptability, and continuous learning, which will support both client work and business growth.
---

## 2.2 Previous Work Experience

### Job 1

| Field | Details |
|---|---|
| Job / Role |**Technician** |
| Name of company | **ER Productions** |
| Date range | **Aprill - October 2021** |
| Summary | *Assisted with the setup and operation of laser and special effects equipment for events, developing technical, troubleshooting, and teamwork skills in a fast-paced environment.*|



---

## 2.3 Qualifications and Education

Include:

- GCSEs
- University qualifications
- Relevant qualifications
- Dates and locations
- DBS check details, if required for the business

BEng (Hons) Electrical and Electronic Engineering

Northumbria University – [Graduation Year]

---

## 2.4 Training

Update this section during the programme to show King’s Trust support and other training.

| Year | Training |
|---|---|
| 2026 | King’s Trust Business Essential Course |
| 2026 / 2027 | A2 CofC Drone Certification |
| 2026 / 2027 | Video Editing and Content Creation Training |
| 2026 / 2027 | Advanced Drone Systems and Robotics Development |


---

## 2.5 Hobbies and Interests

- Drone building and flying
- FPV drone cinematography
- Robotics and autonomous systems
- Technology and engineering development
- Video production and cinematic storytelling
- Learning programming and technical systems
- Gaming and futuristic design concepts

---

## 2.7 Additional Information

I am committed to continuously improving both my technical and creative skills while building Zer0Gravity into a long-term business. My long-term vision is to expand beyond drone services into advanced drone systems and future-focused aerial technology development.

---

# Section Three: Products and Services

## 3.1 Are You Selling a Product or Service?

**Service** (with future **Product** development)

---

## 3.2 Describe the Basic Product or Service You Are Going to Sell

I will be selling:

I will provide professional drone photography and videography services for real estate, events, and businesses, creating high-quality aerial content that helps clients market their properties, brands, and events through unique elevated perspectives.

Keep this brief. More detail goes in Section 3.3.

---

## 3.3 Describe the Different Types of Products or Services You Will Sell

| Product or Service | Price | Description |
|---|---:|---|
| `[Product / Service]` | £`[Price]` | `[Description]` |
| `[Product / Service]` | £`[Price]` | `[Description]` |
| `[Product / Service]` | £`[Price]` | `[Description]` |
| `[Product / Service]` | £`[Price]` | `[Description]` |
| `[Product / Service]` | £`[Price]` | `[Description]` |

---

### Minimum Viable Product Notes

Start trading with your Minimum Viable Product/s only.

Your MVP should be:

- Easiest or most affordable to provide
- Easiest to sell
- Realistic for your current resources

For each product or service, include:

- Description
- Price
- What is included
- Duration, if service-based
- Photos, if useful

---

## 3.4 Products or Services Not Sold at the Start

If you are selling everything from Section 3.3 at the start, write `N/A`.

| Product or Service | Price | Month You Will Start Selling | Description |
|---|---:|---|---|
| `[Product / Service]` | £`[Price]` | `[Month]` | `[Description]` |

---

## 3.5 Additional Information

`[Add any extra information here]`

---

# Section Four: The Market

## 4.1 Are Your Customers Businesses or Individuals?

`[Businesses / Individuals / Both]`

---

## 4.2 Describe Your Typical Customer

### Individual Customer Profile

Consider:

- Age range
- Married or single
- Gender
- Location
- Income level
- Occupation
- Education level
- Interests and hobbies
- Values and beliefs
- Personality traits
- Lifestyle
- Buying motivations
- Buying concerns
- Buying channels
- Product usage
- Where they currently shop or buy similar services

`[Write customer profile here]`

---

### Business Customer Profile

Consider:

- Company size
- Revenue level
- Number of employees
- Location
- Stage of growth
- Technology level
- Budget
- Company goals

`[Write business customer profile here]`

---

### Decision Makers

Consider:

- Job role / function
- Demographics
- Pain points
- Key KPIs
- Buying process

`[Write decision maker profile here]`

---

## 4.3 Where Are Your Customers Based?

In Year 1, my customers are based in the UK. I will focus on these cities / areas:

- `[Area 1]`
- `[Area 2]`
- `[Area 3]`

Tip: Always think big but start small. Start with an area you know well first, get established, then expand slowly.

---

### Website Traffic

I will drive traffic to my website via:

- `[Channel 1]`
- `[Channel 2]`
- `[Channel 3]`

If selling online, identify websites or platforms that will send traffic to your store or service.

---

## 4.4 What Prompts Your Customers to Buy Your Product or Service?

A prompt is an event or need that causes a customer to look for your product or service.

1. `[Prompt 1]`
2. `[Prompt 2]`
3. `[Prompt 3]`
4. `[Prompt 4]`
5. `[Prompt 5]`

Examples:

- Practical need
- Special occasion
- Everyday use
- Need a gift
- A problem that needs solving

---

## 4.5 What Factors Help Customers Choose Which Business to Buy From?

Consider:

- Price
- Quality
- Word of mouth
- Reviews
- Customer service
- Sustainability
- Experience
- Credentials

List at least five:

1. `[Factor 1]`
2. `[Factor 2]`
3. `[Factor 3]`
4. `[Factor 4]`
5. `[Factor 5]`

---

## 4.6 Have You Sold Products or Services Already?

Update as you go along.

### Trade Event or Sales Channel 1

**Name:** `[Name]`  
**Date:** `[Date]`  
**Sales:** £`[Amount]`  
**Costs:** £`[Amount]`  
**Profit:** £`[Amount]`

### Trade Event or Sales Channel 2

**Name:** `[Name]`  
**Date:** `[Date]`  
**Sales:** £`[Amount]`  
**Costs:** £`[Amount]`  
**Profit:** £`[Amount]`

### Trade Event or Sales Channel 3

**Name:** `[Name]`  
**Date:** `[Date]`  
**Sales:** £`[Amount]`  
**Costs:** £`[Amount]`  
**Profit:** £`[Amount]`

---

## 4.7 Have You Got Customers Waiting to Buy Your Product or Service?

`[Add details here]`

Mention:

- Contracts
- Written commitments
- Pre-orders
- Website ready to take orders
- Existing interest

---

## 4.8 Additional Information

List where people can find out more about your business.

| Platform | Details |
|---|---|
| Website | `[Website]` |
| Instagram | `@[handle]`, `[follower count]` |
| TikTok | `@[handle]`, `[follower count]` |
| Mailing list subscribers | `[Number]` |

---

# Section Five: Market Research

## 5.1 Key Findings from Desk Research

Refer to your market research slides.

### Market Size

`[Write findings here]`

### Market Trends

`[Write findings here]`

### Important Facts, Statistics, or Events

`[Write findings here]`

### Seasonality and External Factors: PESTLE

`[Write findings here]`

### Knowledge of Target Areas

Research the areas where your customers are based. Include:

- Population size
- Population age
- Economic trends
- Income trends
- Relevance to your business

| Area | Relevant Research Point |
|---|---|
| `[Area 1]` | `[Research point]` |
| `[Area 2]` | `[Research point]` |
| `[Area 3]` | `[Research point]` |

---

### Industry Knowledge

`[Write at least one strong paragraph showing your knowledge of the industry]`

Useful research sources:

- Office for National Statistics
- Business & IP Centre, British Library
- SBREC / COBRA database
- Statista
- Google Trends
- Google Alerts
- AnswerThePublic
- Sector magazines and directories
- Previous work experience
- Your own industry knowledge

---

## 5.2 Key Findings from Field Research: Customer Questionnaire

### Questionnaire Sample

**How did you choose the people who completed the questionnaire, and how many people completed it?**  
`[Write 1–2 sentences here]`

---

### Overall Outcome

`[Write 2–4 sentences here]`

---

### Data Points

1. `[Data point 1]`
2. `[Data point 2]`
3. `[Data point 3]`
4. `[Data point 4]`
5. `[Data point 5]`

Examples:

- 90% have previously bought children’s clothes online and would do so again.
- 70% of those surveyed aged between 30–35 would like to buy gender-neutral children’s clothes.

---

### Conclusions

`[Write 2–8 sentences explaining conclusions and any changes to your business]`

---

### Testimonials and Reviews

`[Add testimonials or reviews here]`

---

### Survey Tips

Aim for:

- 100 surveys sent to your target audience
- 50 responses
- 30+ responses from your target audience confirming the product/service, quality, and price are right

Free platforms:

- Tally.so
- Typeform
- SurveyMonkey
- Google Forms
- Microsoft Forms

---

## 5.3 Key Findings from Field Research: Test Trading

I registered my business in `[Month Year]` and started trading in `[Month Year]`.

| Item | Amount |
|---|---:|
| Total Turnover | £`[Amount]` |
| Total Costs | £`[Amount]` |
| Total Profit | £`[Amount]` |

---

### Market Research

`[Did test trading reflect your desk research and surveys? What did you apply from competitor research and SWOT analysis?]`

---

### Marketing

`[Key learnings, challenges, consistency, results]`

---

### Operations

`[Key learnings, challenges, supply chain, legal compliance, payment, delivery, what worked and what did not]`

---

### Pricing

`[Key learnings, challenges, customer willingness to buy, pricing adjustments]`

---

### Back-Up Plan

`[Suppliers, skills, training, pivot options]`

---

## 5.4 Additional Information

`[Add extra information here]`

---

# Section Six: Marketing Strategy

Base your strategy on:

- Market research from Section 5
- Competitor analysis
- Ideal customer research
- Industry standards

Consider completing a free marketing audit using ChatGPT.

---

## Marketing SMART Goals

`[Write SMART marketing goals here]`

---

## Marketing Channels to Consider

- Face-to-face marketing
- Direct sales
- Markets
- Pop-ups
- Conferences
- Website
- SEO
- Email
- Blogs
- Facebook
- TikTok
- WhatsApp
- Instagram
- LinkedIn
- Partnerships
- Affiliate marketing
- Marketplaces
- Referral scheme
- Customer loyalty scheme
- Guest blogs
- Guest podcasts
- Networking events
- Business cards
- Flyers
- Content marketing
- Paid ads

---

## Marketing Strategy Table

| What are you going to do and when? | Why have you chosen this method? | How will you do this? | How much will it cost? | Measure of success |
|---|---|---|---:|---|
| Website / SEO / Blog | `[Reason]` | `[Method]` | £`[Cost]` | `[Measure]` |
| Instagram | `[Reason]` | `[Method]` | £`[Cost / Free]` | `[Measure]` |
| Reviews | `[Reason]` | `[Method]` | £`[Cost]` | `[Measure]` |
| Email list | `[Reason]` | `[Method]` | £`[Cost]` | `[Measure]` |
| Google Business Profile | `[Reason]` | `[Method]` | £`[Cost / Free]` | `[Measure]` |
| Markets / events | `[Reason]` | `[Method]` | £`[Cost]` | `[Measure]` |
| Referrals | `[Reason]` | `[Method]` | £`[Cost]` | `[Measure]` |

---

## Measurement Ideas

### Face-to-Face Marketing

- Footfall
- Conversations
- Leads captured
- Conversion rate
- Revenue per event
- Customer feedback

### Direct Sales Outreach

- Number of contacts
- Open rate
- Reply rate
- Meetings booked
- Win rate
- Pipeline created

### Website / SEO / Blogs

- Organic traffic
- Search rankings
- Click-through rate
- Bounce rate
- Time on page
- Conversion rate

### Email Marketing

- Open rate
- Click-through rate
- Conversion rate
- Unsubscribe rate
- List growth

### Social Media

- Reach
- Engagement rate
- Saves
- Shares
- Comments
- Follower growth
- Profile link clicks
- Video completion rate

### Referrals

- Number of referrals
- Referral conversion rate
- Cost per referral
- Repeat customers

---

# Section Seven: Competitor Analysis

## 7.1 Table of Competitors

Research competitor:

- Website
- Social media
- Prices
- Services
- Strengths
- Weaknesses
- Size / longevity
- Reviews
- SEO ranking
- Customer experience

| Name, Location, and Business Size | Product or Service Offered | Price | Strengths | Weaknesses |
|---|---|---:|---|---|
| `[Competitor 1]` | `[Service]` | £`[Price]` | `[Strengths]` | `[Weaknesses]` |
| `[Competitor 2]` | `[Service]` | £`[Price]` | `[Strengths]` | `[Weaknesses]` |
| `[Competitor 3]` | `[Service]` | £`[Price]` | `[Strengths]` | `[Weaknesses]` |
| `[Competitor 4]` | `[Service]` | £`[Price]` | `[Strengths]` | `[Weaknesses]` |

---

## 7.2 SWOT Analysis

Include how you will reduce weaknesses and threats.

| Strengths | Weaknesses |
|---|---|
| `[Point 1]` | `[Point 1]` |
| `[Point 2]` | `[Point 2]` |
| `[Point 3]` | `[Point 3]` |

| Opportunities | Threats |
|---|---|
| `[Point 1]` | `[Point 1]` |
| `[Point 2]` | `[Point 2]` |
| `[Point 3]` | `[Point 3]` |

---

### Weakness and Threat Mitigation

`[Write a paragraph explaining how you will manage weaknesses and threats]`

---

## 7.3 Unique Selling Point: USP

Your USP should explain what makes your business different and why customers should buy from you.

**USP:**  
`[Write 1–2 sentences here. Try to say it in less than 10 words if possible.]`

---

# Section Eight: Operations and Logistics

## 8.1 Production

### Products

If selling products, explain the production journey from sourcing or design to final product.

- `[Step 1]`
- `[Step 2]`
- `[Step 3]`
- `[Step 4]`
- `[Step 5]`

---

### Customer Journey

- Customer hears about the business through a marketing channel
- Customer makes an enquiry or places an order
- `[Next step]`
- `[Next step]`
- `[Final step]`

---

### Services

Starting point: what happens when a potential customer makes an enquiry?

- Customer reaches out via a marketing channel and requests a quote
- I send an estimate for cost and timescale
- Customer confirms and I arrange a site visit, if needed
- Customer agrees in writing to details and price
- `[Next step]`
- `[Final step]`

---

## 8.2 Delivery to Customers

### Products

If selling products:

`[Explain delivery process here]`

Include:

- Delivery method
- Delivery provider
- Delivery cost
- Whether customer pays extra
- Whether delivery is included in product price

---

### Services

If selling services:

`[Explain how customers receive the service]`

Include:

- How the service is delivered
- Any extra charges
- Delivery of final files, products, or outcomes

---

## 8.3 Payment Methods and Terms

`[Explain payment methods and terms here]`

Include:

- When you get paid
- Payment methods
- Card or payment processing fees
- Deposit or upfront payment
- Amendments / revisions
- Accounts and bookkeeping
- Terms and conditions

---

## 8.4 Suppliers

| Name and Location of Supplier | Item Required and Price | Payment Arrangement | Reason for Choosing Supplier |
|---|---:|---|---|
| `[Supplier]` | £`[Item / Price]` | `[Payment terms]` | `[Reason]` |
| `[Supplier]` | £`[Item / Price]` | `[Payment terms]` | `[Reason]` |
| `[Supplier]` | £`[Item / Price]` | `[Payment terms]` | `[Reason]` |

---

## 8.5 Premises

### Working from Home

`[Write 1–2 sentences here]`

Include:

- Where you will run the business from
- Permission from landlord / mortgage provider, if needed
- Suitability of home workspace

Useful link:

- Running a business from home - GOV.UK

---

### External Premises / Markets / Shared Space

If required:

`[Write 1–4 sentences here]`

Include:

- Size
- Rent
- Rates
- Contract type
- Facilities
- Convenience for customers
- Permits or market fees

---

## 8.6 Equipment

This is not just a shopping list. Only include what the business needs and uses.

| Item | Already Owned? | Maintenance: When and Cost | New or Second-Hand | Purchased From | Price |
|---|---|---|---|---|---:|
| `[Item]` | `[Yes / No]` | `[Details]` | `[New / Second-hand]` | `[Supplier]` | £`[Price]` |
| `[Item]` | `[Yes / No]` | `[Details]` | `[New / Second-hand]` | `[Supplier]` | £`[Price]` |
| `[Item]` | `[Yes / No]` | `[Details]` | `[New / Second-hand]` | `[Supplier]` | £`[Price]` |

---

## 8.7 Transport

`[Explain transport here]`

Include:

- How you will meet customers
- How you will collect stock or equipment
- Monthly cost
- Driving licence requirements
- Business insurance for vehicle
- Vehicle rental, if needed
- Public transport costs

---

## 8.8 Legal Requirements

Research the laws that apply to your business.

Legal requirements to consider:

- Business registration
- HMRC and tax responsibilities
- GDPR
- ICO registration, if applicable
- Cookies policy for website
- Health and safety
- Trading licences
- Returns policies
- Music licence, if applicable
- Sector-specific regulations

### Legal Requirement 1

`[Write 1–2 sentences here]`

### Legal Requirement 2

`[Write 1–2 sentences here]`

### Legal Requirement 3

`[Write 1–2 sentences here]`

### Where I Will Get Legal Advice

`[Write 1–2 sentences here]`

---

## 8.9 Insurance Requirements

Everyone on the Enterprise Programme is required to have basic business insurance.

Common types:

- Public Liability Insurance
- Product Liability Insurance
- Professional Indemnity Insurance
- Contents Insurance
- Goods in Transit Insurance
- Equipment Insurance

| Insurance Type | Supplier | Monthly / Annual Cost | Notes |
|---|---|---:|---|
| `[Insurance]` | `[Supplier]` | £`[Cost]` | `[Notes]` |
| `[Insurance]` | `[Supplier]` | £`[Cost]` | `[Notes]` |
| `[Insurance]` | `[Supplier]` | £`[Cost]` | `[Notes]` |

Useful link:

- moneysupermarket.com/business-insurance

---

## 8.10 Management and Staff

`[Write 1–4 sentences here]`

Include:

- Whether you are working alone in Year 1
- Whether you plan to use freelancers
- Whether you plan to hire staff later
- Who will manage operations
- Who will handle admin, finances, delivery, marketing, and customer service

Useful link:

- ACAS

---

## 8.11 Additional Information

`[Add extra information here]`

